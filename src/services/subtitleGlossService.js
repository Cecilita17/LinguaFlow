 ...
 * 3. Enqueues missing lines in chunks of 15 lines with parallel batch processing
 * 4. Triggers onUpdate callback as each batch finishes so the user sees real-time glossing
 * 5. Provides onProgress callback with exact counts (e.g. 45 / 120 lines glossed)
 */
export function enrichSubtitlesWithGlosses({
  ...
  // Phase 1: Apply offline tokenization & merge cached AI tokens immediately
  ...
  // Identify lines that still need AI glossing
  ...
  const totalToGloss = missingLines.length;

  if (totalToGloss === 0) {
    if (onProgress) {
      onProgress({ total: subtitles.length, completed: subtitles.length, isGlossing: false });
    }
    return prepared;
  }

  if (onProgress) {
    onProgress({ total: totalToGloss, completed: 0, isGlossing: true });
  }
  ...
  // Phase 2: Background batch processing in chunks of 15 lines with concurrency = 2
  const CHUNK_SIZE = 15;
  const chunks = [];
  ...
  (async () => {
    ...
    const processNextChunk = async () => {
      while (chunkIndex < chunks.length) {
        ...
        const aiResults = await fetchBatchGlossesApi(chunk, targetLang, nativeLang, apiKey);
        ...
        if (Array.isArray(aiResults) && aiResults.length > 0) {
          ...
          aiResults.forEach((item, itemIdx) => {
            if (item && Array.isArray(item.tokens) && item.tokens.length > 0) {
              const idx = findMatchingSubtitleIndex(currentSubtitles, chunk, item, itemIdx);
              if (idx !== -1) {
                const subId = currentSubtitles[idx].id;
                const mergedTokens = mergeAiTokensWithSegmented(currentSubtitles[idx].tokens, item.tokens);

                cache[subId] = mergedTokens;
        ...
        if (onProgress) {
          onProgress({
            total: totalToGloss,
            completed: Math.min(completedCount, totalToGloss),
            isGlossing: completedCount < totalToGloss
          });
        }
    ...
    // Run 2 parallel workers for fast processing of the entire subtitle file
    await Promise.all([
      processNextChunk(),
      chunks.length > 1 ? processNextChunk() : Promise.resolve()
    ]);

    if (onProgress) {
      onProgress({ total: totalToGloss, completed: totalToGloss, isGlossing: false });
    }
  })().catch(err => {
